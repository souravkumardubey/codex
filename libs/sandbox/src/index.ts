...
import { LANGUAGE_CONFIG } from '@codex/shared';
import type { SupportedLanguage, SandboxConfig } from '@codex/shared';
...
export async function executeInSandbox(
  ...
  const workspaceId = `codex-${Date.now()}-${Math.random().toString(36).substring(2, 8)}`;
  ...
  try {
    ...
    // Build command - pipe stdin from file when provided
...
Ulimits: [
  { Name: 'nofile', Soft: 1024, Hard: 2048 },
    ...
    // Get container stats for memory
    let memoryUsed = 0;
    try {
