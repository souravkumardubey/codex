...
import { executeInSandbox } from '@codex/sandbox';
import { createLogger } from '@codex/logger';
import type { QueueJobData, TestResult } from '@codex/shared';
...
function buildTestCode(language: string, sourceCode: string): string {
  ...
  const annotated = `${sourceCode}\n\n${style.line} Codex test wrapper — read input from stdin`;
...
export async function processExecutionJob(job: Job<QueueJobData>) {
  const { executionId, language, sourceCode, stdin, testCases } = job.data;
  ...
  try {
    ...
    if (testCases && testCases.length > 0) {
      ...
      await db.submission.updateMany({
        where: { executionId },
        data: {
          status: isCompileError ? 'ERROR' : allPassed ? 'ACCEPTED' : 'REJECTED',
